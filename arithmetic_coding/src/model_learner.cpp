...
ModelLearner::ModelLearner(Schema schema, const CompressionConfig &config)
  ...
  } else if (config_.sort_by_attr_ != -1) {
    ordered_attr_list_.push_back(config_.sort_by_attr_);
    inactive_attr_.insert(config_.sort_by_attr_);
    model_predictor_list_[config_.sort_by_attr_].clear();
  }
  InitActiveModelList();
}

void ModelLearner::FeedTuple(const AttrVector &tuple) {
  for (size_t i = 0; i < active_model_list_.size(); ++i) active_model_list_[i]->FeedTuple(tuple);
}
...
void ModelLearner::EndOfData() {
  switch (learner_stage_) {
    case 0:
      ...
      for (size_t i = 0; i < active_model_list_.size(); i++) active_model_list_[i]->EndOfData();
      for (size_t i = 0; i < active_model_list_.size(); i++) StoreModelCost(*active_model_list_[i]);
    ...
    case 1:
      for (size_t i = 0; i < active_model_list_.size(); ++i) {
        ...
        if (selected_model_[target_var] == nullptr ||
            selected_model_[target_var]->GetModelCost() > active_model_list_[i]->GetModelCost())
