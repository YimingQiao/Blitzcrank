...
struct CompressionConfig {
  ...
  // sort_by_attr_ = -1 means no specific sorting required
  int sort_by_attr_;
  bool skip_model_learning_;
  // If skip_model_learning_ flag is true, the following preset dependency will
  // be used
...
/**
 * The ModelLearner class learns all the models simultaneously in an online
 * fashion.
 */
class ModelLearner {
 public:
   ...
   */
  ModelLearner(Schema schema, const CompressionConfig &config);
