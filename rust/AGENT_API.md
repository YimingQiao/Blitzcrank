...
defaults to independent fields, one state and full precision. Explicit `joint`
enables joint ENUM symbols and eligible four-byte STRING chunks. Both profiles
decode one row independently. Precision 8..16 applies to small models; lower
precision changes probabilities/size, never source values. Older binaries reject
the joint flag/chunk tag. Ordinary `compress` stays unchanged.
...
`seek-record-bench ARCHIVE [QUERIES=300000] [TABLE_BITS=0]` measures resident
...
and reports a mean, not p50/p99 or CLI end-to-end latency. TABLE_BITS=0 uses
compact alias models; 1..16 explicitly permits exact bounded direct tables.
...
Optional Cargo features `encode-tables` / `decode-tables` consume additional
128 KiB / 512 KiB per entropy model without changing the archive representation;
`capabilities.lookup_tables` reports whether the executable enables them.
