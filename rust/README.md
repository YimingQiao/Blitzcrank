...
and resident typed random reads. The historical C++ implementation is retained.
...
## Independent records: the default record profile
...
No joint columns, string chunks, large LUTs or decoded-row cache.
...
See [the typed API](RECORD_API.md) and [JSON agent contract](AGENT_API.md).
...
| Joint records | `compress-records INPUT SCHEMA OUTPUT 1 16 joint` | Fewer entropy events; larger models/files possible |
...
| Four-state records | `compress-records INPUT SCHEMA OUTPUT 4 16 balanced` | Explicit parallelism/space tradeoff |
...
See [SIMD scope, memory budget and format](SIMD.md). Neither wide SIMD nor extra
states are assumed to help single-row reads.

Cargo features `encode-tables` / `decode-tables` add 128 KiB / 512 KiB per model
without changing ordinary payload bytes. They are off by default. Probability
precision 8..16 and typed-reader numeric fusion are also explicit experiments.
...
- Historical reports separate resident queries from CSV throughput, preserve
  failures and report space costs. Joint-profile numbers are **not** default
  balanced-profile numbers; no universal 4x/10x claim is made.
- Review and tests do not constitute a security audit. See
  [release review](RELEASE_REVIEW.md), [record measurements](benchmarks/RANDOM_ACCESS.md)
  and [paper-data inventory](benchmarks/PAPER_RESULTS.md).
...
Old preview archives remain readable; old binaries reject newer flags/codecs.
These formats are not yet a frozen long-term interchange standard.
See the [base format reference](FORMAT.md).

[Record format extensions](RECORD_API.md) document joint groups and string chunks;
[SIMD format extensions](SIMD.md) document cumulative DC64 bulk streams.
