...
use delayed_coding::{DecodeModel, DecodeValueModel, Decoder, Model};
...
pub struct RecordReader<'a> {
    ...
    pub(crate) fused: Vec<Option<DecodeValueModel>>,
}
impl RecordReader<'_> {
    /// Experimental numeric dictionary/alias-slot fusion; payload unchanged.
    pub fn with_fused_numeric(mut self) -> Result<Self> {
        self.fused = self
            .entropy
            .iter()
            .map(|f| {
                if let Mapping::Decimals(values) = &f.mapping {
                    Ok(Some(DecodeValueModel::new(
                        f.model,
    ...
    /// Opt-in exact lookup tables. Worst case: 8 * 2^max_bits bytes per
    /// entropy column, additional to the archive's original model storage.
    pub fn with_direct_tables(mut self, max_bits: u32) -> Self {
        self.models = self
            .entropy
            .iter()
            .map(|f| DecodeModel::from_model(f.model).with_direct_table(f.model, max_bits))
            .collect();
        self
    }
        ...
        self.prefixes.capacity() * std::mem::size_of::<Prefix>()
            ...
            + self
                ...
                .map(|f| match &f.mapping {
                    ...
                    Mapping::Chunks(v) => v.capacity() * 4,
                    Mapping::Group { columns, decoded } => {
                        columns.capacity() * std::mem::size_of::<usize>() + decoded.capacity() * 8
                    }
                    _ => 0,
                })
                .sum::<usize>()
            ...
            + self
                ...
                .sum::<usize>()
            + self.fused.capacity() * std::mem::size_of::<Option<DecodeValueModel>>()
            + self
                .fused
                .iter()
                .flatten()
                .map(DecodeValueModel::memory_bytes)
                .sum::<usize>()
    }
    ...
    fn decode<const LANES: usize>(&self, payload: &[u8], out: &mut Record) -> Result<()> {
        ...
        let buffered = LANES == 4 && self.dictionary_only && self.fused.is_empty();
        if buffered {
        ...
        for (i, (field, model)) in self.entropy.iter().zip(&self.models).enumerate() {
            let c = field.column;
            if let Some(Some(model)) = self.fused.get(i) {
                let n = f64::from_bits(decoder.read_value(model)?);
                out.values[c] = if n.is_nan() {
                    Value::Null
                } else {
                    Value::Decimal(n)
                };
            ...
            let symbol = if matches!(field.mapping, Mapping::Bytes(_) | Mapping::Chunks(_)) {
                0
            } else if buffered {
                out.symbols[i]
            } else {
            ...
            let value = match &field.mapping {
                Mapping::Group { columns, decoded } => {
                    let ids = &decoded[symbol as usize];
                    for (&c, &id) in columns.iter().zip(ids) {
                        out.values[c] = Value::Category(u32::from(id));
                    }
                    continue;
                }
                Mapping::Chunks(words) => {
                    let start = out.text.len();
                    let len = out.lengths[c];
                    ensure(start + len <= 64 << 20, "record text limit")?;
                    out.text.reserve(len + 3);
                    for _ in 0..len.div_ceil(4) {
                        out.text
                            .extend_from_slice(&words[decoder.read_prepared(model)? as usize]);
                    }
                    ensure(
                        out.text[start + len..].iter().all(|&b| b == 0),
