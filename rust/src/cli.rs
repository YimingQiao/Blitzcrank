...
impl Input {
    ...
    fn encode_records(
        ...
        bits: u32,
        joint: bool,
    ) -> blitzcrank_rs::Result<Vec<u8>> {
        match self {
            Self::Fast(t) if joint => blitzcrank_rs::compress_joint_records(t, lanes, bits),
            Self::Fast(t) => blitzcrank_rs::compress_records(t, lanes, bits),
            Self::General(t) => general::compress_records(t, lanes, bits, joint),
        }
    }
}
...
fn capabilities() -> Value {
    json!({"name":"blitzcrank-rs", "version":env!("CARGO_PKG_VERSION"),
        ...
        "lookup_tables":{"encode":cfg!(feature="encode-tables"),"decode":cfg!(feature="decode-tables"),
            "extra_encode_bytes_per_model":131072,"extra_decode_bytes_per_model":524288},
        "commands":{
            "compress":{"arguments":["input_csv","schema","output_archive"],"optional":["block_rows=256","states=1"]},
            "compress-records":{"arguments":["input_csv","schema","output_archive"],"optional":["states=1","small_alphabet_precision=16","profile=balanced"],"profiles":["balanced","joint"],"block_rows":1},
            ...
            "seek-record-bench":{"arguments":["input_archive"],"optional":["queries=300000","table_bits=0"],"typed":true,"setup_included":false}},
...
fn run(args: &[String], delimiter: u8, force_general: bool) -> ApiResult<Value> {
    match args.first().map(String::as_str) {
        ...
        Some("compress-records") if (4..=7).contains(&args.len()) => {
            let lanes = args.get(4).map(|s| s.parse()).transpose().context("E_USAGE")?.unwrap_or(1);
            let bits = args.get(5).map(|s| s.parse()).transpose().context("E_USAGE")?.unwrap_or(16);
            if !matches!(lanes, 1|4) || !(8..=16).contains(&bits) { return Err(failure("E_USAGE", "expected states 1 or 4 and precision 8..16")); }
            let profile = args.get(6).map(String::as_str).unwrap_or("balanced");
            if !matches!(profile, "balanced" | "joint") { return Err(failure("E_USAGE", "expected profile balanced or joint")); }
            let mut output = AtomicOutput::new(&args[3])?;
            let begin = Instant::now();
            let config = fs::read_to_string(&args[2]).map_err(io_failure)?;
            let table = input(&args[1], &config, delimiter, force_general)?;
            let parsed = Instant::now();
            let bytes = table.encode_records(lanes, bits, profile == "joint").context("E_DATA")?;
            let encoded = Instant::now();
            output.file.write_all(&bytes).map_err(io_failure)?;
            output.commit()?;
            Ok(json!({"rows":table.rows(),"bytes":bytes.len(),"block_rows":1,"states":lanes,"small_alphabet_precision":bits,"profile":profile,"joint_symbols":profile=="joint","string_chunks":profile=="joint",
        ...
        Some("seek-record-bench") if (2..=4).contains(&args.len()) => {
            let queries = args.get(2).map(|s| s.parse()).transpose().context("E_USAGE")?.unwrap_or(300000usize);
            let bits = args.get(3).map(|s| s.parse()).transpose().context("E_USAGE")?.unwrap_or(0u32);
            if queries == 0 || queries > 10000000 || bits > 16 { return Err(failure("E_USAGE", "expected queries 1..10000000 and table_bits 0..16")); }
            ...
            let reader = if bits == 0 { reader } else { reader.with_direct_tables(bits) };
