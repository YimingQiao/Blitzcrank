...
the last row has a newline, bit 2 enables joint categorical models.
...
Frequencies sum to 65,536. Joint metadata, if flagged, follows all columns
([specification](RECORD_API.md)). Then `(block_count + 1)` u32 offsets: first
...
order, followed by one mixed-model alias DC16 stream. Without joint models,
enum events follow field order. With joint models, one event per group follows
group order. No per-column payload lengths in record mode.

## V2 (`BLTZRS02`)

Header: eight-byte magic, u8 flags, u8 delimiter (comma/pipe), u8 states (1/4),
then u32 columns, rows and block rows. Flags 0/1/2 have the same meanings as v1;
bit 3 is the explicit [SIMD bulk extension](SIMD.md).
...
- 4 string chunks: record-only; see [record extension](RECORD_API.md).

Joint metadata follows columns when flagged. Then `ceil(rows/block_rows)+1`
...
byte/chunk events in field order, excluding grouped enums, followed by joint
events. Fixed columns contribute no entropy events.
...
recomputed CRC. Callers must bound resources; see README and RELEASE_REVIEW.
