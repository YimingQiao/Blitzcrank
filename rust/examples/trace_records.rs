...
fn main() -> Result<()> {
    ...
    if !(3..=6).contains(&args.len())
        || args[3..]
            .iter()
            .any(|s| s != "verify-all" && s != "fused" && !s.starts_with("tables="))
    {
        return Err("trace_records ARCHIVE TRACE_U32_LE [verify-all] [tables=0..16]".into());
    }
    let verify_all = args[3..].iter().any(|s| s == "verify-all");
    let table_bits = args[3..]
        .iter()
        .find_map(|s| s.strip_prefix("tables="))
        .map(str::parse::<u32>)
        .transpose()?;
    if table_bits.is_some_and(|b| b > 16) {
        return Err("invalid table bits".into());
    }
    ...
    if bytes.starts_with(b"BLTZRS02") {
        ...
        let reader = if args[3..].iter().any(|s| s == "fused") {
            reader.with_fused_numeric()?
        } else {
            reader
        };
        let reader = if let Some(bits) = table_bits {
            reader.with_direct_tables(bits)
        } else {
            reader
        };
