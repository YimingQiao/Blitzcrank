...
pub fn compress(table: &Table, options: Options) -> Result<Vec<u8>> {
    ...
    out.push(u8::from(table.crlf) | (u8::from(table.final_newline) << 1));
        ...
        if block_rows == 1 {
            ...
            for (column, model) in table.columns.iter().zip(&models) {
                match column {
                    ...
                    Column::Enum { ids, .. } => events.push(Event {
...
impl<'a> Archive<'a> {
    pub fn open(bytes: &'a [u8]) -> Result<Self> {
        ...
        ensure(flags & !3 == 0, "unknown flags")?;
    ...
    pub fn record_reader(&self) -> Result<crate::record::RecordReader<'_>> {
        ...
        for (column, spec) in self.specs.iter().enumerate() {
            match spec {
                ...
                Spec::Enum { model, .. } => entropy.push(EntropyField {
                    column,
                    model,
