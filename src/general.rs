...
pub fn compress(table: &GeneralTable, block_rows: usize, lanes: usize) -> Result<Vec<u8>> {
    compress_config(table, block_rows, lanes, false)
}
...
pub fn compress_simd(table: &GeneralTable, block_rows: usize) -> Result<Vec<u8>> {
    ...
    compress_config(table, block_rows, 1, true)
}
...
fn compress_config(
    ...
    out.push(u8::from(table.crlf) | (u8::from(table.final_newline) << 1) | (u8::from(simd) << 3));
    ...
    for (c, (column, kind)) in table.columns.iter().zip(&table.kinds).enumerate() {
        ...
        match column {
            ...
            Column::Bytes { counts, .. } => {
                ...
                let model = Model::from_counts(counts.as_slice())?;
        ...
        if block_rows == 1 {
            ...
            for (column, model) in table.columns.iter().zip(&models) {
                match column {
                    ...
                    Column::Dictionary { ids, .. } => events.push(Event {
                        model,
...
impl<'a> GeneralArchive<'a> {
    pub fn open(bytes: &'a [u8]) -> Result<Self> {
        ...
        let lanes = r.byte()? as usize;
            ...
            flags & !11 == 0 && matches!(delimiter, b',' | b'|') && matches!(lanes, 1 | 4),
            "invalid options",
        )?;
        let columns = r.word()?;
        let rows = r.word()?;
        let block_rows = r.word()?;
            ...
            flags & 8 == 0 || (block_rows >= 4096 && lanes == 1),
    ...
    pub fn record_reader(&self) -> Result<crate::record::RecordReader<'_>> {
        ...
        for (column, (spec, &kind)) in self.specs.iter().zip(&self.kinds).enumerate() {
            match spec {
                ...
                Spec::Dictionary { model, tokens } => entropy.push(EntropyField {
                    column,
                    model,
        ...
        Ok(RecordReader {
            dictionary_only: entropy
                .iter()
                .all(|f| !matches!(f.mapping, Mapping::Bytes(_))),
            models: entropy
                .iter()
    ...
    fn record<const LANES: usize>(&self, payload: &[u8], b: &mut GeneralBlock) -> Result<()> {
        ...
        for (c, spec) in self.specs.iter().enumerate() {
            match spec {
                Spec::Fixed { .. } => (),
                Spec::Dictionary { model, .. } => b.ids[c].push(decoder.read(model)?),
                Spec::Bytes { model, .. } => {
