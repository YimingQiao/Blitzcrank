...
#[test]
fn removed_experimental_formats_are_rejected() {
    ...
    let t = Table::read_csv(Cursor::new("a\nb\n"), &Schema::parse("ENUM 2 0").unwrap()).unwrap();
    ...
    let t = general::GeneralTable::read(Cursor::new("a\nb\n"), vec![general::Kind::Text], b',')
        .unwrap();
    let original = general::compress(&t, 1, 1).unwrap();
    for (position, value) in [(8, original[8] | 4), (24, 4)] {
        let mut bytes = original.clone();
        bytes[position] = value; // joint flag or removed first-column chunk codec
        repair(&mut bytes);
        assert!(GeneralArchive::open(&bytes).is_err());
    }
}
