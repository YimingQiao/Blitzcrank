...
#[test]
fn independent_byte_strings_roundtrip_and_typed_output() {
    let mut input = String::new();
    for i in 0..66000 {
    ...
    for lanes in [1, 4] {
        let bytes = general::compress(&table, 1, lanes).unwrap();
        let archive = GeneralArchive::open(&bytes).unwrap();
        assert_eq!(archive.column_codec(0), Some("dc_bytes"));
...
#[test]
fn independent_categorical_records_preserve_every_field() {
    use blitzcrank_rs::{compress, Archive, Options, Schema, Table};
    for input in [
        "1,x,a,b,c,z\n-2,y,a,d,c,w\n3,x,e,b,f,z\n",
        "0,x,a,b,c,z\n0,x,a,b,c,z\n",
    ] {
...
#[test]
fn mixed_independent_record_event_order_and_corruption() {
