...
/*! \def RAPIDJSON_DEFAULT_ALLOCATOR
    ...
    \brief Allows to choose default allocator_.
...
/*! \def RAPIDJSON_DEFAULT_STACK_ALLOCATOR
    \ingroup RAPIDJSON_CONFIG
    \brief Allows to choose default stack allocator_ for Document.
...
// select candidates according to nested encoding and allocator_ types
...
//! Represents a JSON value. Use Value for UTF8 encoding and default allocator_.
/*!
    A JSON value can be one of 7 types. This class is a variant type supporting
    these types.

    Use the Value if UTF8 and default allocator_
...
public:
    ...
    */
    ...
    //! Explicit copy constructor (with allocator_)
    /*! Creates a copy of a Value by using the given Allocator
        \tparam SourceAllocator allocator_ of \c rhs
    ...
    */
    ...
    /*! \note Requires the definition of the preprocessor symbol \ref RAPIDJSON_HAS_STDSTRING.
     */
    GenericValue(const std::basic_string<Ch>& s, Allocator& allocator_) : data_() { SetStringRaw(StringRef(s), allocator_); }
#endif
    ...
    */
    ...
    /*! \param name A string value as name of member.
        \param value constant string reference as value of member.
        \param allocator_    Allocator for reallocating memory. It must be the same one as used before. Commonly use GenericDocument::GetAllocator().
        \return The value itself for fluent API.
        \pre  IsObject()
        \note This overload is needed to avoid clashes with the generic primitive type AddMember(GenericValue&,T,Allocator&) overload below.
        \note Amortized Constant time complexity.
    */
    GenericValue& AddMember(GenericValue& name, std::basic_string<Ch>& value, Allocator& allocator_) {
        GenericValue v(value, allocator_);
        return AddMember(name, v, allocator_);
    }
#endif
    ...
    */
    ...
    //! Set this value as a string by copying from source string.
    /*! \param s source string.
        \param allocator_ Allocator for allocating copied buffer. Commonly use GenericDocument::GetAllocator().
        \return The value itself for fluent API.
        \post IsString() == true && GetString() != s.data() && strcmp(GetString(),s.data() == 0 && GetStringLength() == s.size()
        \note Requires the definition of the preprocessor symbol \ref RAPIDJSON_HAS_STDSTRING.
    */
    GenericValue& SetString(const std::basic_string<Ch>& s, Allocator& allocator_) { return SetString(StringRef(s), allocator_); }
#endif
...
private:
    ...
    Map* &DoReallocMap(Map** oldMap, SizeType newCapacity, Allocator& allocator_) {
        Map **newMap = static_cast<Map**>(allocator_.Malloc(GetMapLayoutSize(newCapacity)));
        GetMapCapacity(*newMap) = newCapacity;
        if (!oldMap) {
            *newMap = new (allocator_.Malloc(sizeof(Map))) Map(MapLess(), allocator_);
        }
        else {
            *newMap = *oldMap;
    ...
    RAPIDJSON_FORCEINLINE Member* DoAllocMembers(SizeType capacity, Allocator& allocator_) {
        return GetMapMembers(DoReallocMap(0, capacity, allocator_));
    }

    void DoReserveMembers(SizeType newCapacity, Allocator& allocator_) {
        ObjectData& o = data_.o;
        if (newCapacity > o.capacity) {
            Member* oldMembers = GetMembersPointer();
            Map **oldMap = oldMembers ? &GetMap(oldMembers) : 0,
                *&newMap = DoReallocMap(oldMap, newCapacity, allocator_);
...
public:
    ...
    /*! Creates an empty document of specified type.
        ...
        \param allocator        Optional allocator_ for allocating memory.
        \param stackCapacity    Optional initial capacity of stack in bytes.
        \param stackAllocator   Optional allocator_ for allocating memory for stack.
    */
    ...
    */
    ...
    ~GenericDocument() {
        ...
        // with an allocator_ like MemoryPoolAllocator (CrtAllocator does not
    ...
    */
    ...
    //! Get the allocator_ of this document.
...
#if RAPIDJSON_HAS_STDSTRING
    GenericObject AddMember(ValueType& name, std::basic_string<Ch>& value, AllocatorType& allocator_) const { value_.AddMember(name, value, allocator_); return *this; }
#endif
